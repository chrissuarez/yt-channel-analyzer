...
Default bind `127.0.0.1:8765`. Discovery view lists topics → expand to
...
Or do it from the UI: `serve-review-ui` has a **Refine** stage (4th step in the stepper) — auto-picked sample table you can edit (remove rows / add by video ID or URL), a "Fetch transcripts & estimate" button, and a "Run refinement ($X.XX)" button (`--real`/`--stub` toggle; `--real` needs `RALPH_ALLOW_REAL_LLM=1` server-side and pops a confirm with the dollar estimate). It polls to completion, then shows the **proposal-review screen** below the sample: pending `taxonomy_proposals` grouped by run (newest first, subtopics under their parent topic), each with an Accept / Reject button — Accept creates the real `topics`/`subtopics` node (parent resolved through any renames; a deleted parent is reported and the proposal rejected) — and a **before → after** sanity panel per sampled episode (the transcript-grade reassignments, with a "✗ wrong" control per topic that reuses the existing mark-wrong path). After accepting, re-run `discover` (taxonomy-aware — it won't downgrade the `refine` rows) to spread the accepted nodes across the rest of the channel; `refine`-source episodes show a "transcript-checked" pill in the topic map.
