...
- [ ] Curation actions: rename topic, merge two topics, split a topic, move episode between subtopics, mark assignment as wrong *(rename happy path done Ralph iteration 8 — `/api/discovery/topic/rename`; merge done Ralph iteration 9 — `/api/discovery/topic/merge` re-points video_topics + subtopics with target-wins collision handling; split done Ralph iteration 10 — `/api/discovery/topic/split` creates a new topic, re-points selected video_topics, drops orphan video_subtopics under source for moved videos; move/mark-wrong + curation-survives-rerun deferred)*
