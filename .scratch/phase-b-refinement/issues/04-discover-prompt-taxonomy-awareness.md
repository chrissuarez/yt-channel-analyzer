...
Status: DONE — branch `feat/issue-04-discover-taxonomy-aware`. `discovery.py`: `DISCOVERY_PROMPT_VERSION` → `discovery-v5`; `_render_discovery_prompt` emits a "Taxonomy already curated for this channel — reuse these exact names …" block from `context["taxonomy"]` (`[{topic, subtopics:[…]}]`; skipped when absent/empty); `_DISCOVERY_SYSTEM` gains a reuse-the-names rule. `run_discovery` loads the project's current `topics` + `subtopics` (rename-resolved — `rename_topic` updates `topics.name` directly) into that list and passes it through the extended `_call_llm_with_optional_correlation`, which now also forwards a `taxonomy` kwarg when the callable's signature accepts it (same opt-in shape as `correlation_id`, so the suite's bare `def f(_videos)` fixtures are untouched); `discovery_llm_via_extractor.call` puts it in the extractor context; `stub_llm` accepts + ignores it (stays deterministic). Never-downgrade: both `video_topics` and `video_subtopics` `ON CONFLICT … DO UPDATE SET` wrap `assignment_source` / `confidence` / `reason` in `CASE WHEN <table>.assignment_source IN ('refine','manual') THEN <existing> ELSE excluded.… END`; `discovery_run_id` is still always updated, so a re-proposed pair of any source remains subject to `_suppress_wrong_assignments_in_run` for the current run and brand-new pairs still INSERT `'auto'`. 8 new tests in `test_discovery.py` (`DiscoveryTaxonomyAwarenessRenderTests` ×2, `DiscoveryRerunTaxonomyAwarenessTests` ×2, `DiscoveryNeverDowngradeRefineTests` ×4). Gate 342 green; `test_transcripts.py` untouched. ROADMAP §B4 ticked; WORKLOG entry. No schema / CLI / cheatsheet change.
Type: AFK
